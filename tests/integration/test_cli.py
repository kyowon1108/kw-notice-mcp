...
def test_metadata_only_cli_blocks_page_denial_after_one_request(
    tmp_path: Path,
) -> None:
    """Given page-one denial, CLI makes no robots request or notice mutation."""
    database = tmp_path / "notices.sqlite3"
    wire = FakeRobotsWire(WireResponse(403, {"content-type": "text/html"}, b"denied"))
...
def test_metadata_only_cli_requests_only_first_page_and_returns_success(
    tmp_path: Path,
) -> None:
    """Given a page-one fixture, CLI collects only first-page metadata."""
    database = tmp_path / "notices.sqlite3"
    list_html = (
