...
CUSTOM_404_TITLE = "HTTP 404 요청하신 페이지가 존재하지 않습니다."
...
@pytest.mark.parametrize(
    "body",
    [
        CUSTOM_404.replace(b"commingsoon.jpg", b"commingsoon.jpG"),
        CUSTOM_404.replace(b'  <meta charset="utf-8">\n', b""),
        CUSTOM_404 + b"<!-- extra -->",
    ],
)
def test_one_byte_missing_marker_or_extra_content_mutation_is_blocked(
    body: bytes,
) -> None:
    """Given any mutation of the complete observed document, fail closed."""
    result = parse_robots_response(200, "text/html", body)

    assert result.block_reason is RobotsBlockReason.INVALID_CONTENT_TYPE
    assert result.mode is None


def test_same_title_spoof_and_arbitrary_html_are_blocked() -> None:
    """Given title mimicry or unrelated HTML, fail closed."""
    spoof = (
        f"<html><head><title>{CUSTOM_404_TITLE}</title></head><body>spoof</body></html>"
    ).encode()
    arbitrary = b"<html><title>Success</title></html>"

    for body in (spoof, arbitrary):
        result = parse_robots_response(200, "text/html", body)
        assert result.block_reason is RobotsBlockReason.INVALID_CONTENT_TYPE
        assert result.mode is None
