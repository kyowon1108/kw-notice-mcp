...
def direct_text(node: Tag | None) -> str:
    """Extract only the immediate plain-text children of one optional HTML node.

    Comments, CDATA, and processing instructions all subclass ``NavigableString``
    and are matched by a ``string=True`` search, so this filters on the exact
    type the way BeautifulSoup's own ``get_text`` does.
    """
    if node is None:
        return ""
    parts = tuple(
        ...
        if type(value) is NavigableString
    )
